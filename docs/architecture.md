...
flowchart LR
    ...
    subgraph Backend["FastAPI Backend (backend/app)"]
        ...
        SVC --> TRAFFIC[Traffic Service<br/>app/traffic]
    ...
    TRAFFIC --> TOMTOM_FLOW[TomTom Traffic<br/>live flow + incidents]
    TRAFFIC --> OSRM
...
6. **Traffic layer** (`backend/app/traffic`) — resolves live/crowd/historical traffic per stop through a fallback hierarchy (TomTom live → QTrace crowd telemetry → historical baseline → unavailable) and blends it into QPSO's cost matrix; never queried from inside QPSO's fitness loop. See [docs/TRAFFIC_ARCHITECTURE.md](TRAFFIC_ARCHITECTURE.md).
7. **Optimization layer** (`backend/app/optimization`) — a QPSO stop-ordering solver (`qpso.py`, random-key encoding over intermediate stops) plus a planned OR-Tools classical baseline, operating on the distance/time matrix a `RoutingProvider` returns. For a plain origin→destination request there is only one possible order, so the optimization service reports `algorithm="DIRECT_ROUTE"` honestly instead of invoking QPSO on a problem with nothing to search (CLAUDE.md §15, §67). A separate, additive multi-vehicle pipeline (`clustering.py`, `greedy_route.py`, `two_opt.py`) serves `FleetOptimizationService` (`POST /api/v1/fleet/routes`): a dependency-free sweep algorithm (Gillett & Miller) partitions destinations across the available fleet by compass bearing from the depot and capacity — giving each vehicle one directionally-coherent wedge instead of criss-crossing routes — then each vehicle's stops are ordered with nearest-neighbor construction + 2-Opt refinement — classical algorithms chosen deliberately for this phase; see [docs/qisa-roadmap.md](qisa-roadmap.md) for the planned Simulated-Annealing-based per-vehicle solver. This pipeline does not use or modify `QPSOSolver`.
8. **Persistence layer** (`backend/app/models`, `backend/app/repositories`) — SQLAlchemy models for the domain entities (User, Vehicle, Depot, DeliveryStop, Route, OptimizationJob, TrafficSnapshot) backed by PostgreSQL/PostGIS, with Alembic migrations.
9. **Background jobs** (`backend/app/workers`) — Redis + RQ workers for long-running optimization jobs; not yet wired up since the current origin→destination feature resolves synchronously (CLAUDE.md §15, §25, §31 - don't add infrastructure before it's needed).
