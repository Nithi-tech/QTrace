"""TrafficTelemetryService - the ingestion half of the QTrace crowd-traffic system
(CLAUDE.md #6.2). Owns validation, map-matching, and snapshot/historical-profile
recomputation. Never calls a routing/matching provider more than once per ingested
batch (one match_trace() call per POST /api/v1/traffic/observations request).
"""
...
from app.traffic.aggregation import classify_freshness, compute_confidence, congestion_score, median_speed_mps
...
# Sanity bounds (CLAUDE.md #38, traffic-free-system master-prompt #43) - reject
# physically implausible input before it ever reaches map-matching or storage.
MAX_PLAUSIBLE_SPEED_MPS = 55.0  # ~198 km/h
MAX_USABLE_ACCURACY_M = 100.0


def _as_utc(at: datetime) -> datetime:
    """SQLite (used in unit tests, CLAUDE.md #44) does not round-trip timezone info the
    way PostgreSQL does, so a value read back from the DB can come back naive even
    though it was always stored as UTC (CLAUDE.md #40) - treat a naive value as UTC
    rather than guessing some other zone."""
...
class TrafficTelemetryService:
    ...
    async def ingest_observations(
        ...
        # Per-segment latest (by the observation's own GPS timestamp, not ingestion
        # wall-clock time) - used to bucket the historical profile correctly even when
        # a batch is ingested well after it was recorded.
        ...
        for leg in match_result.legs:
            ...
            if start_obs is None or end_obs is None:
                continue
            # The phone's own reported speed is the primary signal (more accurate than
            # back-computing from two GPS fixes) - see app/schemas/map_matching.py.
    ...
    def _update_snapshot(self, repository: TrafficRepository, segment_id: str, now: datetime) -> None:
        ...
        if not recent:
            # Nothing to summarize yet - no snapshot row is created (CLAUDE.md #49:
            # absence of data must not be reported as UNKNOWN=LOW).
            return
        ...
        reference = resolve_reference_speed(
            profile_speed_mps=segment.profile_speed_mps if segment else None,
        ...
        confidence = compute_confidence(len(recent), self._min_observations, status)
    ...
    def _update_historical_profile(
        ...
        """Bucketed by the observation's own GPS timestamp (CLAUDE.md #40) - a batch
        uploaded late, or backfilled, still lands in the historical bucket it actually
        happened in, not whenever the server happened to process it."""
